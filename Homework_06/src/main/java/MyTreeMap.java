...
public class MyTreeMap<K extends Comparable<K>, V> {
    ...
    private class Node {
        ...
        Node right;
        int size;
        int height; // добавлена высота узла

        public Node(K key, V value) {
            this.key = key;
            this.value = value;
            size = 1;
            height = 0; //добавлена высота поля при создании
        }
    }
    ...
    public int height() { // вернуть высоту узла
        return height(root);
    }

    private int height(Node node) { // приватная часть метода
        if (node == null) {
            return 0;
        }
        return node.height;
